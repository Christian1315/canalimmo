$(function() {
	...
	$('.next').on('click', function() {
		$('.slide-page').css({'margin-left': '-20%' })
		bullet[current - 1].classList.add('active');
		progress.css('width', '50%');
		current += 1;
	});
	$('.next-1').on('click', function() {
		$('.slide-page').css({'margin-left': '-40%' });
		bullet[current - 1].classList.add('active');
	...
	$('#ptypes_sale')[0].parentNode.classList.add('d-none', 'invisible');
	$('#price_sale')[0].classList.add('d-none', 'invisible');

	$('#status').on('change', function() {
		if($('#status')[0].value == "1" ) {
			$('#ptypes_rent')[0].parentNode.classList.remove('d-none', 'invisible');
			$('#ptypes_sale')[0].parentNode.classList.add('d-none', 'invisible');
			$('#price_rent')[0].classList.remove('d-none', 'invisible');
			$('#price_sale')[0].classList.add('d-none', 'invisible');
		} 
		else {
			$('#ptypes_sale')[0].parentNode.classList.remove('d-none', 'invisible');
			$('#ptypes_rent')[0].parentNode.classList.add('d-none', 'invisible');
			$('#price_sale')[0].classList.remove('d-none', 'invisible');
			$('#price_rent')[0].classList.add('d-none', 'invisible');
		}
	})

	$('#images').on('click', function() {
		$('#images input')[0].click();
	})

	$('#images input').on('change', ({target})=>{
		var files = target.files
		if(files.length && files.length < 6) {
			$('#images span')[0].classList.remove('text-danger');
			$('#images span')[0].innerHTML = `${files.length} Fichiers chargées sur 6 / ${totalSize(files)}`;
		}
		else {
			$('#images span')[0].innerHTML = `${files.length} Fichiers chargées au lieu de 6`;
			$('#images span')[0].classList.add('text-danger');
		}
	})
	...
	function totalSize(files) {
		...
		return (sum / 1000) >= 1000 ? `${(sum / 1000000).toFixedDown(2)} MB` : `${(sum / 1000).toFixedDown(2)} KB`;
	}
